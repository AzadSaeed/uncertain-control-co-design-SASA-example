...
switch upper(opts.UP.arch)
    ...
    case 'SH'
        ...
        if auxdata.case == 5           % Use Polytopic uncertainties
            opts.UP.method = "MC_WCPOLYTOPE";
            for i =1:length(auxdata.Vert)
                auxdata.i = i;
                tic
                [F,T,U,Y,P,in,opts] = InnerLoop(auxdata,opts,[],[],[],i);
                Ctime = toc;
                sol.F = F;
                sol.T = T;
                sol.U = U;
                ...
                savename = strcat("sol_MC_SH_Det_WCpoly",string(auxdata.i),".mat");
            ...
            [~, idx] = min(Obj);
            [F,T,U,Y,P,in,opts] = InnerLoop(auxdata,opts,[],[],[],idx);
            sol.F = F;
            sol.T = T;
            sol.U = U;
            sol.Y = Y;
            ...
            sol.time = Ctime;
            Foldername = strcat('Case', string(auxdata.case));
            savename = strcat("sol_MC_SH_Det_WCpoly",".mat");
            pathsol = msavename(mfilename('fullpath'),strcat('Solutions/',Foldername)); %#ok<NASGU>
            save(fullfile(pathsol, savename), 'sol')
            %out = sol;

        elseif auxdata.case == 6 % Use Polytopic uncertainties for various sizes of uncertainties
        ...
        else            % worst-case robust with and without penalty
            tic
...
if auxdata.case ~=5 && auxdata.case ~=6 && auxdata.case ~=7
...
if nargin == 0
    ...
    switch pcase
        ...
        case 2                                                             % SE-UCCD using gPC
            ...
            opts.PC          = (opts.d_i(1)+1)*(opts.d_i(2)+1)*...         % N-D polynomial overall order
                (opts.d_i(3)+1); 
